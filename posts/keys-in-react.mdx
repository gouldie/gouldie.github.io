...
Use the unique identifier as the key rather than the index. In the above code editor, try giving
each product a unique id and assigning that id as the key for each `<Product />`.
